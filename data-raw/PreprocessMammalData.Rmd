...
attr(finalModel, "SE") <- matrix(0.0, 2, 629)
logLik(finalModel)

names(attributes(finalModel))

tips <- dtMammals[list(as.integer(attr(finalModel, "tree")$tip.label)), SpeciesName2]

X2 <- values[, tips]
attr(finalModel, "X") <- X2

logLik(finalModel)

treeFinalModel <- attr(finalModel, "tree")

tree2 <- PCMTreeSetRegimes(
  tree,
...
attr(finalModel, "X") <- values[, tree2$tip.label]
attr(finalModel, "tree") <- tree2 # treeFinalModel
attr(finalModel, "SE") <- SEs[, tree2$tip.label]*0

metaI <- PCMInfo(attr(finalModel, "X"), attr(finalModel, "tree"), finalModel, SEs[, tree2$tip.label])

PCMLik(attr(finalModel, "X"), attr(finalModel, "tree"), finalModel, SEs[, tree2$tip.label])

logLik(finalModel)

#PCMLik(valuesMammals, tree2, finalModel)
#PCMLik(values[, tree2$tip.label], tree2, finalModel)
```
