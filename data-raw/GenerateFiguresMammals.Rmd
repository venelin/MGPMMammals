...
# load result file from inferring MGPM on the mammal principal component data.
load("Results/MGPM_A_F_best_clade_2_2_PRCValues/FinalResult_MGPM_A_F_best_clade_2_2_PRCValues.RData")
bestFitToPRCDataWithSEs <- RetrieveBestFitScore(fitMappings)
usethis::use_data(bestFitToPRCDataWithSEs, overwrite = TRUE)
```
...
```{r fig3-phylopic-annotation-tips, dev.args=list(colormodel="rgb"), dpi=1200, fig.width=7.58, fig.height=8.24, eval=FALSE}
...
```{r fig-fan-tree-and-data-prc, fig.width=3.43, fig.height=1.72, eval=TRUE}
inferredModel <- bestFitToPRCDataWithSEs$inferredModel
inferredMapping <- attr(inferredModel, "mapping")
inferredTree <- attr(bestFitToPRCDataWithSEs$inferredModel, "tree")
...
palettePRC <- PCMColorPalette(
  PCMTreeNumParts(inferredTree), as.character(PCMRegimes(inferredTree)))
  
plTree <- PCMTreePlot(
  inferredTree, layout = 'fan', open.angle = 4, size= .25, palette = palettePRC)
...
dtTraitsReal <- data.table(
  ...
  regime = PCMTreeGetPartRegimes(inferredTree)[
    PCMTreeGetPartsForNodes(
      inferredTree, seq_len(PCMTreeNumTips(inferredTree)))],
  x = t(MGPMMammals::prcObject$x)[1, seq_len(PCMTreeNumTips(inferredTree))],
  y = t(MGPMMammals::prcObject$x)[2, seq_len(PCMTreeNumTips(inferredTree))]
)
...
cowplot::plot_grid(plTree, plTraitsReal, scale = c(1.08, 1), labels = LETTERS[1:2], label_size = 10)
```

```{r calculateClusterMGPMPRCvsMGPMSTAR}
treeMGPMPRC <- attr(MGPMMammals::bestFitToPRCDataWithSEs$inferredModel, "tree")
treeMGPMSTAR <- attr(MGPMMammals::bestFitToDataWithSEs$modelOptim, "tree")
  
matMGPMPRC <- PCMTreeMatrixNodesInSameRegime(treeMGPMPRC)
mode(matMGPMPRC) <- "double"

matMGPMSTAR <- PCMTreeMatrixNodesInSameRegime(treeMGPMSTAR)
mode(matMGPMSTAR) <- "double"

perf_Cluster = c(
  tpr = TruePositiveRate(matMGPMPRC, matMGPMSTAR),
  fpr = FalsePositiveRate(matMGPMPRC, matMGPMSTAR))
```
